...
function(GetGitState _working_dir)
    ...
    if (NOT exit_code EQUAL 0)
        set(ENV{GIT_LAST_TAG} "None")
    else()
        set(ENV{GIT_LAST_TAG} "${output}")
    endif()

    RunGitCommand(branch --show-current ${object})
    if (NOT exit_code EQUAL 0)
        set(ENV{GIT_CURRENT_BRANCH} "None")
    else()
#        message(STATUS "GIT COMMAND rev-parse gives ${output}")
        set(ENV{GIT_CURRENT_BRANCH} "${output}")
    endif()
...
#    message(STATUS "CHECK GIT ON WORKING DIR = ${_working_dir}")
...
#            message(STATUS "State didn't change")
            return()
        endif()
    endif()
...
function(SetupGitMonitoring)
#    message(STATUS "SetupGitMonitoring")
#    message(STATUS "-DGIT_WORKING_DIR=${GIT_WORKING_DIR}")
#    message(STATUS "-DGIT_EXECUTABLE=${GIT_EXECUTABLE}")
#    message(STATUS "-DGIT_STATE_FILE=${GIT_STATE_FILE}")
#    message(STATUS "-DPRE_CONFIGURE_FILE=${PRE_CONFIGURE_FILE}")
#    message(STATUS "-DPOST_CONFIGURE_FILE=${POST_CONFIGURE_FILE}")
#    message(STATUS "-DPROJECT_NAME=${PROJECT_NAME}")
#    message(STATUS "git_watcher used=${CMAKE_CURRENT_LIST_FILE}")
    add_custom_target(check_git_${PROJECT_NAME}
        ...
        BYPRODUCTS
            ${POST_CONFIGURE_FILE}
            ${GIT_STATE_FILE}
        COMMENT "Checking the ${PROJECT_NAME} git repository for changes..."
        COMMAND
            ${CMAKE_COMMAND}
...
#        message(STATUS "PROJECT_NAME BUILD TIME: ${PROJECT_NAME}")
        CheckGit("${GIT_WORKING_DIR}" changed)
        if(changed OR NOT EXISTS "${POST_CONFIGURE_FILE}")
            GitStateChangedAction()
#        else()
#            message(STATUS "No change to GIT_WORKING_DIR=${GIT_WORKING_DIR}")
        endif()
    else()
        # >> Executes at configure time.
#        message(STATUS "PROJECT_NAME CONFIGURE TIME: ${PROJECT_NAME}")
        SetupGitMonitoring()
    endif()
endfunction()
